...
if (Rnum($user, $email) != 0){ 
	die ("Duplicate user, enter new user name or email"); 
}
...
{	$user    = $_GET["user"];
	...
	$address = $_GET["addresss"] . ", " . $_GET["city"] . ", " . $_GET["state"] . " " . $_GET["zipcode"] ;
	$cell    = $_GET["cell"];
	$major   = $_GET["major"];
...
$s="insert into REGISTERED values ('$user', '$email', '$pwd', '$fullname', '$cell','$address', 'NOW()', '$major', '$numcourse')";  
...
($y = mysql_query ( $x  )) or die ( mysql_error() );
	while ($r = mysql_fetch_array($y))
	{
		...
		$cell = $r["cell"];
		$address = $r["address"];
		$regist_datetime = $r["regist_datetime"];
		$major = $r["major"];
}
